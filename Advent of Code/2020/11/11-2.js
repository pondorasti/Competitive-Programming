  ...
  for (row in grid) {
    for (column in grid[row]) {
      ...
      for (seatIndex in directionsX) {
        ...
        let adjacentX = parseInt(column)
        let adjacentY = parseInt(row)
        while (!reachedEnd) {
          adjacentX += directionsX[seatIndex]
          adjacentY += directionsY[seatIndex]
          if (
            0 <= adjacentX &&
            ...
            if (grid[adjacentY][adjacentX] === "#") {
              occupiedSeats += 1
              reachedEnd = true
            } else if (grid[adjacentY][adjacentX] === "L") {
              reachedEnd = true
            }
          } else {
            reachedEnd = true
      ...
      if (grid[row][column] === "L" && occupiedSeats === 0) {
        newGrid[row][column] = "#"
      } else if (grid[row][column] === "#" && occupiedSeats >= 5) {
